    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleAddDemande} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Produit demandé *</label>
            <select
            ...
            >
              ...
              {products.filter(p => (p.quantite_restante || 0) > 0).map(product => (
                <option key={product.id} value={product.id}>
                  {product.nom} (Stock: {product.quantite_restante} {product.unite?.label})
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            {formData.produit_id && (
              <p className="text-xs text-gray-500 mt-1">
                Stock disponible: {products.find(p => p.id === parseInt(formData.produit_id))?.quantite_restante || 0} {products.find(p => p.id === parseInt(formData.produit_id))?.unite?.label}
              </p>
            )}
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded-xl">
            ...
            <p className="text-sm text-blue-800">
              Votre demande sera soumise à validation. Les administrateurs et employés de production peuvent valider les demandes.
            </p>
          </div>
